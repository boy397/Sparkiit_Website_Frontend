...
export default function GeneralSettings() {
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setStatus({ type: 'success', msg: 'General settings updated' });
    ...
    return (
        <div className="max-w-4xl mx-auto py-10">
            <div className="mb-10">
                <h1 className="text-3xl font-black text-white mb-2 uppercase tracking-tight">General <span className="text-[#00875a]">Settings</span></h1>
                <p className="text-white/40 text-sm">Manage your site name and basic contact information.</p>
            </div>

            <form onSubmit={handleSave} className="bg-white/[0.03] border border-white/5 rounded-3xl p-10 shadow-2xl">
                <div className="grid grid-cols-1 gap-8">
                    <InputField label="Site Name" value={formData.siteName} onChange={(val: string) => setFormData({...formData, siteName: val})} placeholder="e.g. SparkIIT" />
                    <InputField label="Contact Email" value={formData.contactEmail} onChange={(val: string) => setFormData({...formData, contactEmail: val})} placeholder="support@sparkiit.com" />
                    <InputField label="Contact Phone" value={formData.contactPhone} onChange={(val: string) => setFormData({...formData, contactPhone: val})} placeholder="+91 1234567890" />
                </div>
